...
这是“已删除路径未被运行时采集”的结构性结果，不代表仍运行路径的实际观测，也不应作为回放实证。
...
- 自动化回归全部通过，遍历基线无变化。
- 结构性结果：`recoverPlayerOccupiedIdentityForPublicReveal` 与 `insertUnknownPlaceholderIntoPile` 的运行时实现已删除；schema 中对应旧名称输出 0 只表示“已删除/未采集”，不能当作仍运行路径的回放观测。
- 实际回放观测：三段累计 263.309 秒样本中，仍在运行的阶段 1 interop 与并存期公共区 swap 兼容路径均为零触发。
- 生产源码净增 122 行，尚未达到“净删代码可观”的 GO 条件。
- 回放来源、完整性及观虚、权变、鹰视、诫厉、同区展示覆盖情况未知。

依据计划中的 G1 标准，最终决定 **NO-GO / 收缩**：保留阶段 1 的匿名牌堆作为终点，不推进阶段
2 至阶段 6。该决定不是因为性能或正确性回退，而是：已删除路径的“零触发”不能当作实证支持，仍运行路径在有限样本中也未观察到冲突收益，同时阶段 1 仍为净增代码，全面迁移的收益证据不足。

G0/G1 回放采集至此结束，临时浏览器回放探针已退役并完成代码清理。历史统计保留在本报告中；
后续只有出现新的非零旧冲突或高频 interop，并能证明继续迁移可带来可量化净删除时，才另立提案重新评估。

## 9. 探针清理

- 已移除 `window.__DXC_TRAVERSAL__` 浏览器控制入口与长生命周期 session API。
- 已移除 `G0_TRAVERSAL_SITES` / `g0` 固定 schema。
- 已移除生产路径中的 `anonymousSlot:*` 决策打点。
- 已删除对应 browser/session/G0 基线测试；通用 `recordTraversal` 与 `traversalBaseline` 保留。
