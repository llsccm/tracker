...
其他技能只要走同一组 `5->10 / 10->5` 全暗整手序列，也会复用同一套账本。
...
- 诫厉等“手牌进交换区后回牌堆”的路径（目标不是 `ToZone=5`）
...
2. 写入账本 `bySpell[121].batches[4] = { cards, cardCount: 4 }`。
...
- 回手牌（`10 -> 5`）时按 `SpellID + FromID` 取批，不按交换区当前物理顺序猜测。
- 某 SpellID 的两批都取回后清理该技能账本；全部清空后删除房间级 key。
