...
- **执行顺序**：
  1. 通过 [`src/tracker/bridge.ts`](src/tracker/bridge.ts) 导出的 `tracker` 调用 `tracker.initTrackerRoom()`；实际实现位于 [`src/tracker/trackerController.ts`](src/tracker/trackerController.ts)，会销毁可能残留的当前 `trackerRoom`，创建新的 [`src/tracker/Room.ts:Room`](src/tracker/Room.ts)，注册默认移动事件处理器。
...
| **单局 Room 创建** | `tracker.initTrackerRoom`                          | [`src/tracker/trackerController.ts`](src/tracker/trackerController.ts)                                            | `GsCModifyUserseatNtf` 开局后创建全新的 `Room` 实例，注册默认移动事件处理器；此时尚未注册玩家，也尚未挂载视图。 |
| **玩家座位注册**   | `tracker.registerTrackerPlayers`                  | [`src/tracker/trackerController.ts`](src/tracker/trackerController.ts)                                            | 应用 `msg.Infos`，创建 `Player` 实例，写入 `seatIDs`、`size` 与主视角座位，并早期挂载固定手牌容器。 |
| **先手补齐**       | `tracker.setTrackerFirstHand` / `Room.setFirstHand` | [`src/tracker/trackerController.ts`](src/tracker/trackerController.ts) / [`src/tracker/Room.ts`](src/tracker/Room.ts) | `GsCFirstPhaseRole` 到达后写入 `firstID`，更新每个玩家的 `fixedViewId` 并刷新座位覆盖层。     |
...
| **记牌器容器销毁** | `tracker.destroyTrackerRoom`                       | [`src/tracker/trackerController.ts`](src/tracker/trackerController.ts)                                            | 销毁 `Room` 对象并调用 `unmount` 卸载所有面板 DOM 元素，清理所有内存缓存。                    |
