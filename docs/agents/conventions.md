...
- 假设游戏源数据可靠：默认信任宿主游戏协议与运行时派发的源数据结构及字段语义，不再对游戏源数据
  进行严格防御或过度兜底。当游戏源数据本身发生错误或损坏时，记牌器与辅助脚本的后续推导运转已无实际意义；
  过度防御只会掩盖真实的协议异常或将程序错误静默伪装成合法状态。
  - **典型反例**：臆想协议数值（如洗牌协议的 `cardCount`）可能传入超大非法值导致内存耗尽，因而
    试图为其设定人为的“安全验证上限”，并在超限时回退到预测值去兜底对账。游戏源数据是权威事实，
    若源数据本身发生严重错乱，脚本强行兜底运转已无意义，过度防御反而会引入虚假上限并凭空增加复杂性。
